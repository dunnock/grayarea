...
use orchestrator::{orchestrator, message::Message};
...
async fn main() -> anyhow::Result<()> {
    ...
    let in_topics: HashMap<String, TopicChannel> = modules.iter()
        .filter_map(|module| 
            module.input.as_ref().map(|input| (input.topic.clone(), channel::bounded(CHANNEL_SIZE))))
        .collect();
    ...
    for (i, module) in modules.into_iter().enumerate() {
        ...
        if let Ok(topics) = module.topics() {
            ...
            for name in topics {
                let topic = in_topics.get(&name)
                    .ok_or_else(|| anyhow::anyhow!("topic {} is not pointing to any function input", name))?;
                out_topics.push(topic.0.clone());
            }
            orchestra.forward_bridge_rx(i, out_topics)?;
        }
        // Connect module's input to topic
        if let Some(Some(topic)) = module.input.as_ref().map(|input| in_topics.get(&input.topic)) {
